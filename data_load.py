...
def make_data_subset_labels(csv_file, img_path='./data/20bn-jester-v1/', label_file='./data/jester-v1-labels.csv', labels=None):

	'''
	output pandas dataframe: 'ID': img ids, 'label': labeled motion, 'label_id': each label's ID (0~n_classes]
	'''

	df = pd.read_csv(csv_file, header=None, sep=';', names=['ID', 'label']) # customized for the csv_file, data specific customization is needed
	...
	if labels is None:
		labels = pd.read_csv(label_file, header=None, sep=';').values.squeeze().tolist() # customized for the csv_file, data specific customization is needed
...
class Jester20BnDataset(Dataset):
	...
	def __init__(self, csv_file, img_path='./data/20bn-jester-v1/', label_file='./data/jester-v1-labels.csv', labels=None, transform=None):
        """
        Args:
        ...
        self.img_id_label_frame = make_data_subset_labels(csv_file,img_path=self.img_path,label_file=label_file,labels=self.labels)
    ...
    def __getitem__(self, idx):
        ...
        idx_path   = os.path.join(self.img_path, self.img_id_label_frame['ID'][idx])
        image_list = [os.path.join(self.img_path, self.img_id_label_frame['ID'][idx],img) for img in os.listdir(idx_path) if '.jpg' in img or '.png' in img]
        
        images = [mpimg.imread(image_name) for image_name in image_list]
        
        # if image has an alpha color channel, get rid of it
        if(image[0].shape[2] == 4):
            new_images = [image[:,:,0:3] for image in images]
            images     = new_images
        
        label    = self.self.img_id_label_frame['label'][idx]
        label_id = self.self.img_id_label_frame['label_id'][idx]
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
		...
		for ii, image in enumerate(iamges):
			h, w = image.shape[:2]
...
class ToTensor(object):
	...
	def __call__(self, sample):
		...
		for ii, image in enumerate(images):
			...
			images[ii] = torch.from_numpy(image)
...
if __name__ == '__main__':
	df = make_data_subset_labels('./data/jester-v1-train.csv', labels=['Swiping Left', 'Swiping Right'])

	data_transform = transforms.Compose([RandomCrop(100),
										ToTensor()])
