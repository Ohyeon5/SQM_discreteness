...
def train_net(device):
    ...
    dataset = HDF5Dataset('../data', load_data=False, data_cache_size=4, transform=None)

    # Split train/test/validatition set (train:70%, val:10%, test:20%)
    # n_dataset = len(transformed_dataset)
    # train_set, val_set, test_set = torch.utils.data.random_split(transformed_dataset, (
    #     n_dataset - (floor(n_dataset * 0.2) + floor(n_dataset * 0.1)), floor(n_dataset * 0.1), floor(n_dataset * 0.2)))
